 ...
 *
 */
public class Exercise6Solution implements Serializable {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -5826269722224341154L;
	private Logger log = LoggerFactory.getLogger(Exercise6Solution.class);
	...
	public void run() throws IOException, InterruptedException, AccumuloException, AccumuloSecurityException, TableExistsException, TableNotFoundException {
		...
		AccumuloInputFormat.setConnectorInfo(job, ExerciseConstants.USER,  new PasswordToken(ExerciseConstants.PASSWORD));
		AccumuloInputFormat.setZooKeeperInstance(job, accumulo.getClientConfig());
		AccumuloInputFormat.setInputTableName(job, ExerciseConstants.RECORD_TABLE);
		...
		JavaRDD<Tweet> tweetRdd = kvRdd.map(entry -> bytesToTweet(entry._2.get()));

		for (Tweet tweet : tweetRdd.collect()) {
			log.info(tweet.getIdStr());
		}
	...
	public static void main(String[] args) throws IOException, InterruptedException, AccumuloException, AccumuloSecurityException, TableExistsException, TableNotFoundException {
		Exercise6Solution exercise = new Exercise6Solution();
		exercise.run();
	}
